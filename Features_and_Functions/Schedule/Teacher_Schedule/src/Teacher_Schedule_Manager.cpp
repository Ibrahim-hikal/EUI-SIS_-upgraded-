...
std::string TeacherScheduleManager::assign_color_for_course(const std::string& course_code) {
    std::vector<std::string> palette = {
        "#e8f5e9", "#e3f2fd", "#fff3e0", "#fce4ec", "#f3e5f5", "#e0f7fa", "#f0f4c3", "#ffe0b2", "#ffcdd2"
    };
...
void TeacherScheduleManager::parse_slots(const std::string& code, const std::string& slots_string, const std::string& slot_type) {
    // Regex matches: "Sunday 09:00-11:00"
    ...
    for (std::sregex_iterator i = words_begin; i != words_end; ++i) {
        ...
        entry.color_hex = assign_color_for_course(code);
...
void TeacherScheduleManager::load_teacher_schedule(const std::string& teacher_name) {
    ...
    if (!file.is_open()) {
        std::cerr << "CRITICAL ERROR: Failed to open Offered_Courses.csv" << std::endl;
        return;
    }
    ...
    while (std::getline(file, line)) {
        ...
        // Ensure row has enough columns (Instructor=4, Lec=5, Tut=6)
        if (cols.size() >= 7) {
            std::string code = cols[0];
            std::string instructor = cols[4];

            // If this teacher teaches this course, extract all slots
            if (instructor == teacher_name) {
...
void TeacherScheduleManager::initUI(Main_App* ui) {
    ...
    for (const auto& entry : current_schedule) {
        ...
        slot.course_name = slint::SharedString("");
        slot.slot_type = slint::SharedString(entry.slot_type);
